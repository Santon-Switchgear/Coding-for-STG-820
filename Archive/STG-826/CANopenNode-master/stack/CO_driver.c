...
//    {
        ...
        //CO_CANsendToModule(CANmodule, buffer, txBuff);
			...
			hcan.pTxMsg->StdId = buffer->ident;
			hcan.pTxMsg->DLC = buffer->DLC;
			for ( index=0; index<buffer->DLC; index++ )
				hcan.pTxMsg->Data[index] = buffer->data[index];
...
void CO_CANinterrupt_Rx(CO_CANmodule_t *CANmodule)
{
	...
	CAN1_RxMsg.DLC = hcan.pRxMsg->DLC;
	CAN1_RxMsg.ExtId = hcan.pRxMsg->ExtId;
	CAN1_RxMsg.FMI = hcan.pRxMsg->FMI;
	CAN1_RxMsg.IDE = hcan.pRxMsg->IDE;
	CAN1_RxMsg.ident = hcan.pRxMsg->StdId;
	CAN1_RxMsg.RTR = hcan.pRxMsg->RTR;
	for ( index = 0; index < CAN1_RxMsg.DLC; index++ )
	  CAN1_RxMsg.data[index] = hcan.pRxMsg->Data[index];
